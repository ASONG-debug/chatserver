...
bool Redis::connect()
{
    ...
    //*****
    if (publish_context_->err)
    {
        cerr << "connect redis failed: " << publish_context_->errstr << endl; // 这里会打印真正的错误原因
        redisFree(publish_context_);
    ...
    //****
    if (subscribe_context_->err)
    {
        cerr << "connect redis failed: " << subscribe_context_->errstr << endl;
        redisFree(publish_context_);
        redisFree(subscribe_context_);
        publish_context_ = nullptr;
...
// 向redis指定的通道channel发布消息 (通道号 消息内容)
...
// 向redis指定的通道subscribe订阅消息 (只做订阅，不接收通道消息)
